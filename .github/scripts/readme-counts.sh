...
ls "$root/pkgs/omarchy/nix-bin" 2>/dev/null | sort > /tmp/rc-nixbin.txt
grep -rlE '\b(pacman|yay)\b' "$omarchy/share/omarchy/bin" 2>/dev/null |
  xargs -r -n1 basename | sort > /tmp/rc-pac.txt
repl_n=$(comm -12 /tmp/rc-pac.txt /tmp/rc-nixbin.txt | wc -l)
